...
L’API de statistiques d’utilisation de la Géoplateforme permet d’obtenir, entre deux dates, le nombre de requêtes (hits) ayant sollicité la Géoplateforme et le volume de données (octets) que la Géoplateforme a émis en réponse.
...
Elle est interrogeable en méthode GET en étant authentifié via son compte Géoplateforme/cartes.gouv.fr.
...
## 2 - En tant que producteur, consulter les statistiques d’utilisation de ses données
...
### 2.1 - Statistiques par organisme
...
### 2.2 - Statistiques par communauté
...
## 3 - En tant qu'utilisateur des géoservices, consulter ses statistiques de consommation des données en accès restreint
