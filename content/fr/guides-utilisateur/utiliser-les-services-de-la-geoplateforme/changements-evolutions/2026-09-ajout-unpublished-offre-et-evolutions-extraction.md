---
title: Ajout du statut UNPUBLISHED pour les offres + Évolutions de l’API d’Extraction + Corrections de bugs sur l’entrepôt
...
tags:
    ...
    - Entrepôt
eleventyNavigation:
    key: Ajout du statut UNPUBLISHED pour les offres + Évolutions de l’API d’Extraction + corrections de bugs sur l’entrepôt
    order: -20260923
date: 2026-09-23
---
...
- DELETE sur une offre en statut `PUBLISHED` : l’offre passe en `UNPUBLISHING` puis en `UNPUBLISHED` une fois la dépublication terminé. À ce moment, l’offre n’est plus accessible (absent de la réponse au `GetCapabilities` par exemple) mais n’est pas encore supprimé de l’entrepôt
...
Ainsi, si on souhaite supprimer une offre, il faudra faire 2 DELETE successifs sur celle-ci. De même pour supprimer une configuration, il faudra bien faire 2 DELETE sur son offre avant de pouvoir supprimer la configuration.
...
- [Entrepôt] Correction d’un bug à la récupération des <span lang="en">_logs_</span> de vérification lorsque ceux-ci faisaient plus de 2 Mo
...
- [Entrepôt] Correction des valeurs d’utilisation des <span lang="en">_endpoints_</span> affiché dans la description d’un <span lang="en">_datastore_</span> qui pouvait être incohérentes
